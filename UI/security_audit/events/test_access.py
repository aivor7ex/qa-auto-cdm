...
ROWS_COUNT_ACCESS_PAGE = None
# --- Инициализация количества строк для вкладки "Аудит доступа" ---
def _init_rows_count_access(page: Page):
    global ROWS_COUNT_ACCESS_PAGE
    page.reload()
    with wait_for_api_response_with_response(page, '/cycleLogs/count') as resp_info:
        pass
    response = resp_info.value
    ROWS_COUNT_ACCESS_PAGE = response.json()["count"]
...
def skip_if_no_pagination(authenticated_page: Page):
    _init_rows_count_access(authenticated_page)
    if ROWS_COUNT_ACCESS_PAGE is not None and ROWS_COUNT_ACCESS_PAGE <= 10:
        pytest.skip("Пагинация не отображается при количестве записей ≤ 10")

@pytest.fixture(scope="function")
def logs_count(authenticated_page: Page):
    # Перехватываем ответ на запрос /cycleLogs/count при открытии вкладки
    with authenticated_page.expect_response(lambda response: "/cycleLogs/count" in response.url) as resp_info:
        # Если вкладка уже открыта, можно обновить страницу или кликнуть по вкладке заново
        pass  # Если нужно, добавьте клик по вкладке
    response = resp_info.value
    data = response.json()
    return data["count"]

@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_events_tab_navigation_and_url(authenticated_page: Page, credentials):
    """
    Проверяет переход на вкладку "Аудит доступа" в разделе "Аудит безопасности" -> "Журналы регистрации"
    и корректность URL.
    """
    global ROWS_COUNT_ACCESS_PAGE
    page = authenticated_page
    # Шаг 1: Ожидаем полной загрузки сайдбара после авторизации
    ...
    logs_button.click()
    page.wait_for_timeout(300)
    # Шаг 4: Клик по вкладке "Аудит доступа" с ожиданием запроса
    with page.expect_response(lambda response: "/cycleLogs/count" in response.url) as resp_info:
        ...
        expect(access_events_tab).to_be_visible()
        access_events_tab.click()
        page.wait_for_timeout(300)
    response = resp_info.value
    ROWS_COUNT_ACCESS_PAGE = response.json()["count"]
    # Шаг 5: Проверка URL
    expect(page).to_have_url(f"https://{credentials['ip']}/security-audit/events/access")

@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_events_tab_others_not_selected(authenticated_page: Page, credentials):
    """
    Проверяет, что после перехода на вкладку "Аудит доступа" остальные вкладки
    ("События безопасности", "Аудит настроек", "События сервисов") не активны (не выбраны).
    """
    page = authenticated_page
    try:
        # Шаг 1: Ожидаем полной загрузки сайдбара после авторизации
        ...
        logs_button.click()
        page.wait_for_timeout(300)
        # Шаг 4: Клик по вкладке "Аудит доступа"
        access_events_tab = page.get_by_role("tab", name="Аудит доступа ")
        expect(access_events_tab).to_be_visible()
        access_events_tab.click()
        page.wait_for_timeout(300)
        # Шаг 5: Проверка, что вкладка "Аудит доступа" активна
        expect(access_events_tab).to_have_attribute("aria-selected", "true")
        ...
        expect(subsystems_events_tab).to_have_attribute("aria-selected", "false")
    except Exception as e:
        if page:
            page.screenshot(path="UI/error_screenshots/access_events_tab_others_not_selected_error.png")
        raise e

@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_update_button_and_api_call(authenticated_page: Page, credentials):
    """
    Проверяет функциональность кнопки 'Обновить' и связанный с ней API-запрос
    на вкладке "Аудит доступа".
    """
    page = authenticated_page
    try:
        ...
        # Шаг 2: Ожидаем сетевой запрос после клика по кнопке "Обновить"
        ...
        if page:
            page.screenshot(path="UI/error_screenshots/update_button_api_call_access_error.png")
        raise e

@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_download_button_modal_and_close(authenticated_page: Page, credentials):
    """
    ...
    page = authenticated_page
    try:
        # Шаг 1: Находим и кликаем на кнопку "Скачать"
        download_button = page.get_by_text("Скачать")
        expect(download_button).to_be_visible()
        download_button.click()
        page.wait_for_timeout(300)
        # Шаг 2: Проверяем появление окна с заголовком
        modal_title = page.get_by_text("Скачать файл в кодировке UTF-8")
        expect(modal_title).to_be_visible()
        # Шаг 3: Проверяем кликабельность кнопок "Да", "Нет" и крестика
        ...
        # Шаг 4: Нажимаем "Нет" и проверяем, что окно закрылось
        no_button.click()
        page.wait_for_timeout(300)
        expect(modal_title).not_to_be_visible()
    except Exception as e:
        if page:
            page.screenshot(path="UI/error_screenshots/download_modal_access_error.png")
        raise e

@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_pagination_navigation(authenticated_page: Page, credentials, skip_if_no_data, skip_if_no_pagination):
    """
    Проверяет навигацию по страницам: переход вперёд, назад, на последнюю и первую страницу.
    """
    page = authenticated_page
    # Шаг 1: Получаем локатор пагинации
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_page_size_10(authenticated_page: Page, credentials, skip_if_no_data, skip_if_no_pagination):
    """
    Проверяет, что при выборе 10 записей на странице отображается не больше 10 строк (или меньше, если данных меньше).
    """
    _init_rows_count_access(authenticated_page)
    global ROWS_COUNT_ACCESS_PAGE
    assert ROWS_COUNT_ACCESS_PAGE is not None, "ROWS_COUNT_ACCESS_PAGE не определён"
    page = authenticated_page
    ...
    assert row_count == min(10, ROWS_COUNT_ACCESS_PAGE), f"Ожидалось {min(10, ROWS_COUNT_ACCESS_PAGE)} строк, найдено {row_count}"

@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_page_size_30(authenticated_page: Page, credentials, skip_if_no_data, skip_if_no_pagination):
    """
    ...
    row_count = rows.count()
    # Шаг 4: Проверяем соответствие количества строк
    assert row_count == min(30, ROWS_COUNT_ACCESS_PAGE), f"Ожидалось {min(30, ROWS_COUNT_ACCESS_PAGE)} строк, найдено {row_count}"

@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_page_size_50(authenticated_page: Page, credentials, skip_if_no_data, skip_if_no_pagination):
    """
    ...
    row_count = rows.count()
    # Шаг 4: Проверяем соответствие количества строк
    assert row_count == min(50, ROWS_COUNT_ACCESS_PAGE), f"Ожидалось {min(50, ROWS_COUNT_ACCESS_PAGE)} строк, найдено {row_count}"

@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_page_size_100(authenticated_page: Page, credentials, skip_if_no_data, skip_if_no_pagination):
    """
    ...
    row_count = rows.count()
    # Шаг 4: Проверяем соответствие количества строк
    assert row_count == min(100, ROWS_COUNT_ACCESS_PAGE), f"Ожидалось {min(100, ROWS_COUNT_ACCESS_PAGE)} строк, найдено {row_count}"

@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_download_modal_close_by_cross(authenticated_page: Page, credentials, skip_if_no_data):
    """
    Проверяет, что модальное окно скачивания закрывается по нажатию на крестик.
    """
    page = authenticated_page
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_pagination_inactive_buttons(authenticated_page: Page, credentials, skip_if_no_data, skip_if_no_pagination):
    """
    Проверяет, что неактивные кнопки пагинации действительно неактивны и страница не меняется.
    """
    page = authenticated_page
    # Шаг 1: Получаем локаторы пагинации и кнопок
    pagination = page.locator('.MuiTablePagination-root.cdm-pagination')
    expect(pagination).to_be_visible()
    action_buttons = page.locator('.cdm-pagination__action-buttons')
    buttons = action_buttons.locator('button')
    ...
    next_btn = buttons.nth(-2)
    last_btn = buttons.nth(-1)
    # Шаг 2: Проверяем неактивность first и prev на первой странице
    ...
    # Шаг 3: Переходим на последнюю страницу (если возможно)
    if last_btn.is_enabled():
        last_btn.click()
        page.wait_for_timeout(300)
    # Шаг 4: Проверяем неактивность next и last на последней странице
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_no_page_size_menu_if_few_rows(authenticated_page: Page, credentials, skip_if_no_data):
    """
    Проверяет, что меню выбора количества строк не отображается, если строк меньше 10.
    """
    _init_rows_count_access(authenticated_page)
    global ROWS_COUNT_ACCESS_PAGE
    assert ROWS_COUNT_ACCESS_PAGE is not None, "ROWS_COUNT_ACCESS_PAGE не определён"
    page = authenticated_page
    # Шаг 1: Проверяем количество строк
    if ROWS_COUNT_ACCESS_PAGE <= 10:
        # Шаг 2: Проверяем отсутствие меню выбора количества строк
        page_size_button = page.locator('.MuiSelect-root[role="button"]')
        assert not page_size_button.is_visible(), "Меню выбора количества строк не должно отображаться при малом числе строк"
    else:
        # Шаг 3: Пропускаем тест, если строк больше 10
        pytest.skip("Строк больше 10, тест не актуален")

@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_page_size_more_than_rows(authenticated_page: Page, credentials, skip_if_no_data):
    """
    Проверяет, что при выборе количества строк больше, чем есть в таблице, отображаются все строки и UI не ломается (Аудит доступа).
    """
    _init_rows_count_access(authenticated_page)
    global ROWS_COUNT_ACCESS_PAGE
    assert ROWS_COUNT_ACCESS_PAGE is not None, "ROWS_COUNT_ACCESS_PAGE не определён"
    ...
    # Шаг 2: Проверяем возможность выбора 100 строк
    if ROWS_COUNT_ACCESS_PAGE < 100:
        page_size_button = page.locator('.MuiSelect-root[role="button"]')
        expect(page_size_button).to_be_visible()
        page_size_button.click()
        option = page.locator('li[role="option"][data-value="100"]')
        ...
        option.click()
        page.wait_for_timeout(500)
        # Шаг 3: Проверяем количество строк в таблице
        rows = page.locator('.cdm-data-grid tbody tr')
        row_count = rows.count()
        assert row_count == ROWS_COUNT_ACCESS_PAGE, f"Ожидалось {ROWS_COUNT_ACCESS_PAGE} строк, найдено {row_count} при выборе 100 строк"
    else:
        # Шаг 4: Пропускаем тест, если строк 100 или больше
        pytest.skip("Строк 100 или больше, тест не актуален")

@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_update_button_multiple_clicks(authenticated_page: Page, credentials, skip_if_no_data):
    """
    Проверяет, что при многократном клике по кнопке 'Обновить' не возникает ошибок и UI не зависает.
    """
    page = authenticated_page
    # Шаг 1: Находим кнопку 'Обновить'
    update_button = page.get_by_text("Обновить")
    expect(update_button).to_be_visible()
    # Шаг 2: Кликаем по кнопке несколько раз
    for _ in range(5):
        update_button.click()
        page.wait_for_timeout(100)
    # Шаг 3: Проверяем, что кнопка осталась активной
    expect(update_button).to_be_enabled()

@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_buttons_active_no_data(authenticated_page: Page, credentials):
    """
    Проверяет, что при отсутствии данных кнопки 'Скачать' и 'Обновить' активны и не вызывают ошибок.
    """
    page = authenticated_page
    # Шаг 1: Проверяем отсутствие данных
    ...
    # Шаг 2: Проверяем активность кнопок
    update_button = page.get_by_text("Обновить")
    download_button = page.get_by_text("Скачать")
    expect(update_button).to_be_enabled()
    expect(download_button).to_be_enabled()
    # Шаг 3: Кликаем по кнопкам
    update_button.click()
    page.wait_for_timeout(300)
    download_button.click()
    page.wait_for_timeout(300)
    # Шаг 4: Закрываем модалку кликом по координатам (1/4 ширины и высоты экрана)
    ...
    # Шаг 6: Проверяем отсутствие окна с ошибкой
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_no_pagination_and_page_size_menu_if_few_rows(authenticated_page: Page, credentials, skip_if_no_data):
    """
    ...
    # Шаг 1: Проверяем количество строк
    if ROWS_COUNT_ACCESS_PAGE <= 10:
        # Шаг 2: Проверяем отсутствие пагинации и меню выбора количества строк
        pagination = page.locator('.MuiTablePagination-root.cdm-pagination')
        page_size_button = page.locator('.MuiSelect-root[role="button"]')
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_tab_persists_after_reload(authenticated_page: Page, credentials):
    """
    Проверяет, что после обновления страницы остаётся та же вкладка (Аудит доступа).
    """
    page = authenticated_page
    # Шаг 1: Проверяем, что вкладка "Аудит доступа" активна
    access_tab = page.get_by_role("tab", name="Аудит доступа ")
    expect(access_tab).to_have_attribute("aria-selected", "true")
    # Шаг 2: Обновляем страницу
    page.reload()
    page.wait_for_timeout(800)
    # Шаг 3: Проверяем, что вкладка осталась активной
    access_tab = page.get_by_role("tab", name="Аудит доступа ")
    expect(access_tab).to_be_visible()
    expect(access_tab).to_have_attribute("aria-selected", "true")
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_filter_menu_appears(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_filter_date_time_appears(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_filter_date_time_set_and_remove(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_filter_date_time_positive_range(authenticated_page: Page, credentials, skip_if_no_data):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_filter_date_time_future_empty(authenticated_page: Page, credentials, skip_if_no_data):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_filter_date_time_inverted_range_empty(authenticated_page: Page, credentials, skip_if_no_data):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_filter_date_time_exact_no_match(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_filter_date_time_only_from(authenticated_page: Page, credentials, skip_if_no_data):
    """
...
# @pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
# def test_go_to_events_with_500_rows(authenticated_page: Page, credentials, skip_if_no_data):
#     """
#     Переходит по базовому URL страницы событий с параметрами ?filter%5Bpage%5D=1&filter%5BrowsPerPage%5D=500
#     и ожидает загрузки таблицы событий.
#     """
#     go_to_events_with_500_rows(authenticated_page)

@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_filter_date_time_only_to(authenticated_page: Page, credentials, skip_if_no_data):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_filter_positive_critical_error(authenticated_page: Page, credentials):
    """
    Проверяет позитивную фильтрацию по критичности: Ошибка
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_filter_negative_critical_error(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_filter_positive_critical_warn(authenticated_page: Page, credentials):
    """
    Проверяет позитивную фильтрацию по критичности: Предупреждение
    """
    filter_name = "Критичность"
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_filter_negative_critical_warn(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_filter_positive_critical_info(authenticated_page: Page, credentials):
    """
    Проверяет позитивную фильтрацию по критичности: Информация
    """
    filter_name = "Критичность"
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_filter_negative_critical_info(authenticated_page: Page, credentials):
    """
...
# @pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
# def test_page_access_filter_positive_service_csi(authenticated_page: Page, credentials):
#     """
#     Проверяет позитивную фильтрацию по Сервису: CSI
#     """
#     filter_name = "Сервис"
#     severity_text = "CSI"
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_check_filter_by_input_first_row_value(authenticated_page: Page, credentials):
    """
...
# @pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
# def test_page_access_filter_positive_service_csi_insensitive(authenticated_page: Page, credentials):
#     """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_filter_negative_service_qwerty123(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_filter_negative_service_csii(authenticated_page: Page, credentials):
    """
    Проверяет негативную фильтрацию по Сервису: CSII
    """
    filter_name = "Сервис"
    severity_text = "CSII"
    check_filter_by_input_negative_other_values(authenticated_page, filter_name, severity_text)
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_filter_negative_service_special_chars(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_filter_negative_service_long_string(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_filter_negative_service_sql_injection(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_filter_negative_service_spaces(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_filter_positive_first_row_user(authenticated_page: Page, credentials):
    """
...
# @pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
# def test_page_access_filter_positive_user_auto_admin(authenticated_page: Page, credentials):
#     """
#     Проверяет позитивную фильтрацию по Пользователю: auto_admin
#     """
#     filter_name = "Пользователь"
#     severity_text = "auto_admin"
#     check_filter_by_input(authenticated_page, filter_name, severity_text)

# @pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
# def test_page_access_filter_positive_user_auto_admin_insensitive(authenticated_page: Page, credentials):
#     """
#     Проверяет позитивную фильтрацию по Пользователю: AUTO_ADMIN (case-insensitive)
#     """
#     filter_name = "Пользователь"
#     severity_text = "AUTO_ADMIN"
#     check_filter_by_input(authenticated_page, filter_name, severity_text)
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_filter_negative_user_qwerty123(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_filter_negative_user_adminn(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_filter_negative_user_special_chars(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_filter_negative_user_long_string(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_filter_negative_user_sql_injection(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_filter_negative_user_spaces(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_filter_positive_first_row_event(authenticated_page: Page, credentials):
    """
...
# @pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
# def test_page_access_filter_positive_event_cycleLogs_find(authenticated_page: Page, credentials):
#     """
#     Проверяет позитивную фильтрацию по Описанию события: cycleLogs.find
#     """
#     filter_name = "Описание события"
#     severity_text = "cycleLogs.find"
#     check_filter_by_input(authenticated_page, filter_name, severity_text)

# @pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
# def test_page_access_filter_positive_event_not_modified_insensitive(authenticated_page: Page, credentials):
#     """
#     Проверяет позитивную фильтрацию по Описанию события: NOT MODIFIED (case-insensitive)
#     """
#     filter_name = "Описание события"
#     severity_text = "NOT MODIFIED"
#     check_filter_by_input(authenticated_page, filter_name, severity_text)

@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_filter_negative_event_qwerty123(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_filter_negative_event_adminn(authenticated_page: Page, credentials):
    """
    Проверяет негативную фильтрацию по Описанию события: adminn
    """
    filter_name = "Описание события"
    severity_text = "adminn"
    check_filter_by_input_negative_other_values(authenticated_page, filter_name, severity_text)
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_filter_negative_event_special_chars(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_filter_negative_event_long_string(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_filter_negative_event_sql_injection(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_filter_negative_event_spaces(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_sorted_positive_date_first_state(authenticated_page: Page, credentials, skip_if_no_data):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_sorted_positive_date_second_state(authenticated_page: Page, credentials, skip_if_no_data):
    """
...
# @pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
# def test_page_access_sorted_positive_critical_first_state(authenticated_page: Page, credentials, skip_if_no_data):
#     """
...
# @pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
# def test_page_access_sorted_positive_critical_second_state(authenticated_page: Page, credentials, skip_if_no_data):
#     """
...
# @pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
# def test_page_access_sorted_positive_user_first_state(authenticated_page: Page, credentials, skip_if_no_data):
#     """
...
# @pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
# def test_page_access_sorted_positive_user_second_state(authenticated_page: Page, credentials, skip_if_no_data):
#     """
...
# @pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
# def test_page_access_sorted_positive_service_first_state(authenticated_page: Page, credentials, skip_if_no_data):
#     """
...
# @pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
# def test_page_access_sorted_positive_service_second_state(authenticated_page: Page, credentials, skip_if_no_data):
#     """
...
"""------------------------------Сортировка по "Описание события"--------------------------------------- """


# @pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
# def test_page_access_sorted_positive_event_first_state(authenticated_page: Page, credentials, skip_if_no_data):
#     """
#     Проверяет позитивный кейс фильтрации по убыванию (состояние 1) по "Описание события"
#     """
...
# @pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
# def test_page_access_sorted_positive_event_second_state(authenticated_page: Page, credentials, skip_if_no_data):
#     """
#     Проверяет позитивный кейс фильтрации по возрастанию (состояние 2) по "Описание события"
#     """
#     column_name = "Описание события"
#     endpoint_substring = "/api/service/remote/logger-analytics/analytics-server/call/cycleLogs?filter="
#     check_sorting_by_text_column_desc(authenticated_page, column_name, endpoint_substring)
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_access_check_modal_window_info_positive(authenticated_page: Page, credentials, skip_if_no_data):
    """
