...
ROWS_COUNT_SETTINGS_PAGE = None

# --- Инициализация количества строк для вкладки "Аудит настроек" ---
def _init_rows_count_settings(page: Page):
    global ROWS_COUNT_SETTINGS_PAGE
    page.reload()
    with wait_for_api_response_with_response(page, '/cycleLogs/count') as resp_info:
        pass
    response = resp_info.value
    ROWS_COUNT_SETTINGS_PAGE = response.json()["count"]
...
def skip_if_no_pagination(authenticated_page: Page):
    _init_rows_count_settings(authenticated_page)
    if ROWS_COUNT_SETTINGS_PAGE is not None and ROWS_COUNT_SETTINGS_PAGE <= 10:
...
def test_page_settings_settings_events_tab_navigation_and_url(authenticated_page: Page, credentials):
    global ROWS_COUNT_SETTINGS_PAGE
    ...
    try:
        ...
        # Шаг № 4: Клик по вкладке "Аудит настроек" с ожиданием запроса
        with page.expect_response(lambda response: "/cycleLogs/count" in response.url) as resp_info:
            ...
            expect(settings_events_tab).to_be_visible()
            settings_events_tab.click()
            page.wait_for_timeout(300)
        response = resp_info.value
        ROWS_COUNT_SETTINGS_PAGE = response.json()["count"]

        # Шаг № 5: Проверка URL
        expect(page).to_have_url(f"https://{credentials['ip']}/security-audit/events/settings")

    except Exception as e:
        if page:
            page.screenshot(path="UI/error_screenshots/settings_events_tab_navigation_error.png")
        raise e

@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_settings_events_tab_others_not_selected(authenticated_page: Page, credentials):
    """
    Проверяет, что после перехода на вкладку "Аудит настроек" остальные вкладки
    ("События безопасности", "Аудит доступа", "События сервисов") не активны (не выбраны).
    """
    page = authenticated_page
    try:
        # Ожидаем полной загрузки сайдбара после авторизации
        ...
        # Шаг № 4: Клик по вкладке "Аудит настроек"
        settings_events_tab = page.get_by_role("tab", name="Аудит настроек ")
        expect(settings_events_tab).to_be_visible()
        settings_events_tab.click()
        page.wait_for_timeout(300)

        # Шаг № 5: Проверка, что вкладка "Аудит настроек" активна
        expect(settings_events_tab).to_have_attribute("aria-selected", "true")
        ...
        if page:
            page.screenshot(path="UI/error_screenshots/settings_events_tab_others_not_selected_error.png")
        raise e

@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_update_button_and_api_call(authenticated_page: Page, credentials):
    """
    Проверяет функциональность кнопки 'Обновить' и связанный с ней API-запрос
    на вкладке "Аудит настроек".
    """
    page = authenticated_page
    try:
        ...
        if page:
            page.screenshot(path="UI/error_screenshots/update_button_api_call_settings_error.png")
        raise e

@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_download_button_modal_and_close(authenticated_page: Page, credentials):
    """
    ...
    try:
        ...
        if page:
            page.screenshot(path="UI/error_screenshots/download_modal_settings_error.png")
        raise e

@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_download_modal_close_by_cross(authenticated_page: Page, credentials, skip_if_no_data):
    """
    Проверяет, что модальное окно скачивания закрывается по нажатию на крестик (Аудит настроек).
    """
    page = authenticated_page
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_settings_pagination_navigation(authenticated_page: Page, credentials, skip_if_no_data, skip_if_no_pagination):
    """
    Проверяет работу пагинации на вкладке "Аудит настроек": переходы между страницами (вперёд, назад, первая, последняя).
    """
    page = authenticated_page
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_page_size_10(authenticated_page: Page, credentials, skip_if_no_data, skip_if_no_pagination):
    """
    Проверяет, что при выборе 10 записей на странице отображается не больше 10 строк (или меньше, если данных меньше).
    """
    _init_rows_count_settings(authenticated_page)
    global ROWS_COUNT_SETTINGS_PAGE
    assert ROWS_COUNT_SETTINGS_PAGE is not None, "ROWS_COUNT_SETTINGS_PAGE не определён"
    page = authenticated_page
    ...
    assert row_count == min(10, ROWS_COUNT_SETTINGS_PAGE), f"Ожидалось {min(10, ROWS_COUNT_SETTINGS_PAGE)} строк, найдено {row_count}"

@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_page_size_30(authenticated_page: Page, credentials, skip_if_no_data, skip_if_no_pagination):
    """
    ...
    row_count = rows.count()
    # Шаг 4: Проверяем соответствие количества строк
    assert row_count == min(30, ROWS_COUNT_SETTINGS_PAGE), f"Ожидалось {min(30, ROWS_COUNT_SETTINGS_PAGE)} строк, найдено {row_count}"

@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_page_size_50(authenticated_page: Page, credentials, skip_if_no_data, skip_if_no_pagination):
    """
    ...
    row_count = rows.count()
    # Шаг 4: Проверяем соответствие количества строк
    assert row_count == min(50, ROWS_COUNT_SETTINGS_PAGE), f"Ожидалось {min(50, ROWS_COUNT_SETTINGS_PAGE)} строк, найдено {row_count}"

@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_page_size_100(authenticated_page: Page, credentials, skip_if_no_data, skip_if_no_pagination):
    """
    ...
    row_count = rows.count()
    # Шаг 4: Проверяем соответствие количества строк
    assert row_count == min(100, ROWS_COUNT_SETTINGS_PAGE), f"Ожидалось {min(100, ROWS_COUNT_SETTINGS_PAGE)} строк, найдено {row_count}"

# --- Негативные тесты ---

@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_no_page_size_menu_if_few_rows(authenticated_page: Page, credentials, skip_if_no_data):
    """
    Проверяет, что меню выбора количества строк не отображается, если строк меньше 10 (Аудит настроек).
    """
    _init_rows_count_settings(authenticated_page)
    global ROWS_COUNT_SETTINGS_PAGE
    assert ROWS_COUNT_SETTINGS_PAGE is not None, "ROWS_COUNT_SETTINGS_PAGE не определён"
    page = authenticated_page
    if ROWS_COUNT_SETTINGS_PAGE <= 10:
        page_size_button = page.locator('.MuiSelect-root[role="button"]')
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_page_size_more_than_rows(authenticated_page: Page, credentials, skip_if_no_data):
    """
    Проверяет, что при выборе количества строк больше, чем есть в таблице, отображаются все строки и UI не ломается (Аудит настроек).
    """
    _init_rows_count_settings(authenticated_page)
    global ROWS_COUNT_SETTINGS_PAGE
    assert ROWS_COUNT_SETTINGS_PAGE is not None, "ROWS_COUNT_SETTINGS_PAGE не определён"
    ...
    if ROWS_COUNT_SETTINGS_PAGE < 100:
        page_size_button = page.locator('.MuiSelect-root[role="button"]')
        expect(page_size_button).to_be_visible()
        page_size_button.click()
        option = page.locator('li[role="option"][data-value="100"]')
        ...
        rows = page.locator('.cdm-data-grid tbody tr')
        row_count = rows.count()
        assert row_count == ROWS_COUNT_SETTINGS_PAGE, f"Ожидалось {ROWS_COUNT_SETTINGS_PAGE} строк, найдено {row_count} при выборе 100 строк"
    else:
        pytest.skip("Строк 100 или больше, тест не актуален")

@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_pagination_inactive_buttons(authenticated_page: Page, credentials, skip_if_no_data, skip_if_no_pagination):
    """
    Проверяет, что неактивные кнопки пагинации действительно неактивны и страница не меняется (Аудит настроек).
    """
    page = authenticated_page
    pagination = page.locator('.MuiTablePagination-root.cdm-pagination')
    expect(pagination).to_be_visible()
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_update_button_multiple_clicks(authenticated_page: Page, credentials, skip_if_no_data):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_buttons_active_no_data(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_no_pagination_and_page_size_menu_if_few_rows(authenticated_page: Page, credentials, skip_if_no_data):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_tab_persists_after_reload(authenticated_page: Page, credentials):
    """
    Проверяет, что после обновления страницы остаётся та же вкладка (Аудит настроек).
    """
    page = authenticated_page
    settings_tab = page.get_by_role("tab", name="Аудит настроек ")
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_filter_menu_appears(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_filter_date_time_appears(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_filter_date_time_set_and_remove(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_filter_date_time_positive_range(authenticated_page: Page, credentials, skip_if_no_data):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_filter_date_time_future_empty(authenticated_page: Page, credentials, skip_if_no_data):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_filter_date_time_inverted_range_empty(authenticated_page: Page, credentials, skip_if_no_data):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_filter_date_time_exact_no_match(authenticated_page: Page, credentials, skip_if_no_data):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_filter_date_time_only_from(authenticated_page: Page, credentials, skip_if_no_data):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_page__filter_date_time_only_to(authenticated_page: Page, credentials, skip_if_no_data):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_filter_positive_critical_error(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_filter_negative_critical_error(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_filter_positive_critical_warn(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_filter_negative_critical_warn(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_filter_positive_critical_info(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_filter_negative_critical_info(authenticated_page: Page, credentials):
    """
...
# @pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
# def test_page_settings_filter_positive_service_csi(authenticated_page: Page, credentials):
#     """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_check_filter_by_input_first_row_value(authenticated_page: Page, credentials):
    """
...
# @pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
# def test_page_settings_filter_positive_service_csi_insensitive(authenticated_page: Page, credentials):
#     """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_filter_negative_service_qwerty123(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_filter_negative_service_CSII(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_filter_negative_service_special_chars(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_filter_negative_service_long_string(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_filter_negative_service_sql_injection(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_filter_negative_service_spaces(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_filter_positive_first_row_user(authenticated_page: Page, credentials):
    """
...
# @pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
# def test_page_settings_filter_positive_user_admin(authenticated_page: Page, credentials):
#     """
#     Проверяет позитивную фильтрацию по Пользователю: admin
#     """
#     filter_name = "Пользователь"
#     severity_text = "admin"
#     check_filter_by_input(authenticated_page, filter_name, severity_text)
...
# @pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
# def test_page_settings_filter_positive_user_admin_insensitive(authenticated_page: Page, credentials):
#     """
#     Проверяет позитивную фильтрацию по Пользователю: ADMIN (case-insensitive)
#     """
#     filter_name = "Пользователь"
#     severity_text = "ADMIN"
#     check_filter_by_input(authenticated_page, filter_name, severity_text)
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_filter_negative_user_qwerty123(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_filter_negative_user_adminn(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_filter_negative_user_special_chars(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_filter_negative_user_long_string(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_filter_negative_user_sql_injection(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_filter_negative_user_spaces(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_filter_positive_first_row_event(authenticated_page: Page, credentials):
    """
...
# @pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
# def test_page_settings_filter_positive_event_not_found(authenticated_page: Page, credentials):
#     """
#     Проверяет позитивную фильтрацию по Описанию события: Not Found
#     """
#     filter_name = "Описание события"
#     severity_text = "Not Found"
#     check_filter_by_input(authenticated_page, filter_name, severity_text)

# @pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
# def test_page_settings_filter_positive_event_not_found_insensitive(authenticated_page: Page, credentials):
#     """
#     Проверяет позитивную фильтрацию по Описанию события: NOT FOUND (case-insensitive)
#     """
#     filter_name = "Описание события"
#     severity_text = "NOT FOUND"
#     check_filter_by_input(authenticated_page, filter_name, severity_text)
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_filter_negative_event_qwerty123(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_filter_negative_event_alloww(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_filter_negative_event_special_chars(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_filter_negative_event_long_string(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_filter_negative_event_sql_injection(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_filter_negative_event_spaces(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_sorted_positive_date_first_state(authenticated_page: Page, credentials):
    """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_sorted_positive_date_second_state(authenticated_page: Page, credentials):
    """
...
# @pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
# def test_page_settings_sorted_positive_critical_first_state(authenticated_page: Page, credentials):
#     """
...
# @pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
# def test_page_settings_sorted_positive_critical_second_state(authenticated_page: Page, credentials):
#     """
...
# @pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
# def test_page_settings_sorted_positive_user_first_state(authenticated_page: Page, credentials):
#     """
...
# @pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
# def test_page_settings_sorted_positive_user_second_state(authenticated_page: Page, credentials):
#     """
...
# @pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
# def test_page_settings_sorted_positive_service_first_state(authenticated_page: Page, credentials):
#     """
...
# @pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
# def test_page_settings_sorted_positive_service_second_state(authenticated_page: Page, credentials):
#     """
...
# @pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
# def test_page_settings_sorted_positive_event_first_state(authenticated_page: Page, credentials):
#     """
...
# @pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
# def test_page_settings_sorted_positive_event_second_state(authenticated_page: Page, credentials):
#     """
...
@pytest.mark.parametrize("credentials", [("admin", "password")], indirect=True)
def test_page_settings_check_modal_window_info_positive(authenticated_page: Page, credentials, skip_if_no_data):
    """
