#!/usr/bin/env python3
"""
Обработчик для POST /manager/maintenanceUpdateBrp
...
def handle(body: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    try:
        logger.info("maintenanceUpdateBrp: start; body_keys=%s", ",".join(sorted(body.keys())) if isinstance(body, dict) else "<none>")
        ...
        if st != 200:
            return {"result": "ERROR", "message": f"maintenanceUpdateBrp POST failed: {st}"}
        ...
        while True:
            ...
            if st_s != 200:
                return {"result": "ERROR", "message": f"status GET failed: {st_s}"}
            ...
            if time.time() - start_ts > max_wait_s:
                return {"result": "ERROR", "message": "status timeout"}
        ...
        if st_l != 200:
            return {"result": "ERROR", "message": f"statusAndLogs GET failed: {st_l}"}
        if not isinstance(resp_l, dict) or resp_l.get("message") != "OK":
            return {"result": "ERROR", "message": "maintenanceUpdateBrpStatusAndLogs not OK"}

        logger.info("maintenanceUpdateBrp: success")
        return {"result": "OK"}
    except Exception as e:
        logger.exception("maintenanceUpdateBrp: exception")
