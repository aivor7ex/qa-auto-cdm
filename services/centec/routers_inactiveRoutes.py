...
ENDPOINT = "/routers/inactiveRoutes"
...
ROUTE_ITEM_SCHEMA = {
    ...
    "required": ["routeType", "dest", "gateway"]
}
...
def response_json(response):
    ...
    for item in json_data:
        ipaddress.ip_network(item['dest'], strict=False)
        ipaddress.ip_address(item['gateway'])
    return json_data

@pytest.mark.parametrize("param, value", PARAM_SCENARIOS)
def test_inactive_routes_parametrized(api_client, attach_curl_on_fail, param, value):
