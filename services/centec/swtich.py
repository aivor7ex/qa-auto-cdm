...
SWITCH_ITEM_SCHEMA = {
    ...
    "required": ["id", "type", "address", "port", "host"]
}
...
def response_json(response):
    ...
    for sw, item in json_data.items():
        for ip_key in ["address", "host"]:
            if ip_key in item:
                ipaddress.ip_address(item[ip_key])
        if "port" in item:
            port = item["port"]
            assert isinstance(port, int)
            assert 1 <= port <= 65535, f"Port out of range: {port}"
        if "type" in item:
            assert item["type"] in {"telnet", "ssh"}, f"Unexpected type: {item['type']}"
    return json_data

@pytest.mark.parametrize("param, value", PARAM_SCENARIOS)
def test_swtich_parametrized(api_client, attach_curl_on_fail, param, value):
