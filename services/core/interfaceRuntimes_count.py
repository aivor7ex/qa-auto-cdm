"""Tests for the /interfaceRuntimes/count endpoint."""
import pytest
import json
from collections.abc import Mapping, Sequence

ENDPOINT = "/interfaceRuntimes/count"
...
# Осмысленная параметризация для тестирования эндпоинта /interfaceRuntimes/count
PARAMS = [
    ...
    pytest.param({"include_metadata": "true"}, 200, id="P21: include_metadata"),
    pytest.param({"include_metadata": "false"}, 200, id="P22: exclude_metadata"),
    pytest.param({"format": "json"}, 200, id="P23: format_json"),
    pytest.param({"format": "text"}, 200, id="P24: format_text"),
    pytest.param({"format": "xml"}, 200, id="P25: format_xml"),
    pytest.param({"verbose": "true"}, 200, id="P26: verbose_true"),
    pytest.param({"verbose": "false"}, 200, id="P27: verbose_false"),
    pytest.param({"detailed": "true"}, 200, id="P28: detailed_true"),
    pytest.param({"detailed": "false"}, 200, id="P29: detailed_false"),
    
    # --- Тестирование игнорируемых параметров ---
    pytest.param({"limit": "10"}, 200, id="P30: limit_ignored"),
    pytest.param({"offset": "5"}, 200, id="P31: offset_ignored"),
    pytest.param({"sort": "name"}, 200, id="P32: sort_ignored"),
    pytest.param({"page": "1"}, 200, id="P33: page_ignored"),
    pytest.param({"search": "bond"}, 200, id="P34: search_ignored"),
    pytest.param({"q": "interface"}, 200, id="P35: query_ignored"),
    
    # --- Специальные count параметры ---
    pytest.param({"count_only": "true"}, 200, id="P36: count_only"),
    pytest.param({"fast_count": "true"}, 200, id="P37: fast_count"),
    pytest.param({"estimate": "true"}, 200, id="P38: estimate_count"),
    pytest.param({"cache": "true"}, 200, id="P39: cache_enabled"),
    pytest.param({"cache": "false"}, 200, id="P40: cache_disabled"),
    pytest.param({"refresh": "true"}, 200, id="P41: refresh_count"),
    pytest.param({"refresh": "false"}, 200, id="P42: no_refresh"),
    
    # --- Негативные сценарии с некорректными фильтрами ---
    pytest.param({"filter": "invalid_json"}, 200, id="N01: invalid_json_ignored"),
    ...
    pytest.param({"filter": '{"rt_active": true}', "format": "json"}, 200, id="P43: filter_and_format"),
    pytest.param({"filter": '{"name": "bond1"}', "verbose": "true"}, 200, id="P44: filter_and_verbose"),
    pytest.param({"include_metadata": "true", "detailed": "true"}, 200, id="P45: metadata_and_detailed"),
    
    # --- Граничные значения ---
    pytest.param({"unsupported_param": "value"}, 200, id="P46: unsupported_param_ignored"),
]
...
def test_interface_runtimes_count_parametrized(api_client, params, expected_status):
    """
    Основной параметризованный тест для эндпоинта /interfaceRuntimes/count.
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Дополнительная проверка что count неотрицательный
