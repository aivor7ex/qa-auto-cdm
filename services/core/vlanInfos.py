...
VLANINFO_SCHEMA = {
    ...
    "properties": {
        ...
        "MAC": {"type": "string"},
        "l2mode": {"type": "boolean"},
        "stpActive": {"type": "boolean"},
        "forwardDelay": {"type": "integer"},
        "helloInterval": {"type": "integer"},
        "maxAge": {"type": "integer"},
        "priority": {"type": "integer"}
    },
    "required": ["vlanId"],  # name больше не обязательное
    "additionalProperties": False
}

VLANINFOS_SCHEMA = {
    "type": "array",
    "items": VLANINFO_SCHEMA
}
...
# recursive_validate и _print_curl больше не нужны.
...
# --- Параметризация: 35+ осмысленных сценариев ---
PARAMS = [
...
def test_vlanInfos_parametrized(api_client, case, params, expected_status, desc):
    """
    Проверяет устойчивость /vlanInfos к различным query-параметрам и валидность MAC-адресов.
    ...
    try:
        assert response.status_code == expected_status, (
            f"Параметризованный тест '{desc}' упал. "
            f"Ожидался статус-код {expected_status}, получен {response.status_code}"
        )

        if response.status_code == 200:
            data = response.json()
            # 1. Валидация общей схемы (массив объектов)
            validate(instance=data, schema=VLANINFOS_SCHEMA)

            # 2. Дополнительная валидация для каждого элемента
            for item in data:
                # Проверка валидности MAC, если поле присутствует
                if "MAC" in item and item["MAC"] is not None:
                    assert is_valid_mac(item["MAC"]), f"MAC-адрес невалиден: {item['MAC']} в объекте {item}"

    except ValidationError as e:
        curl_command = _format_curl_command(response)
        error_message = (
            f"\nПараметризованный тест '{desc}' упал из-за ошибки валидации схемы: {e.message}\n\n"
        ...
        curl_command = _format_curl_command(response)
        error_message = (
            f"\nПараметризованный тест '{desc}' упал из-за ошибки в данных: {e}\n\n"
            "================= Failed Test Request (cURL) ================\n"
            f"{curl_command}\n"
            "============================================================="
        )
        ...
        curl_command = _format_curl_command(response)
        error_message = (
            f"\nПараметризованный тест '{desc}' упал с неожиданной ошибкой: {e}\n\n"
            "================= Failed Test Request (cURL) ================\n"
            f"{curl_command}\n"
            "============================================================="
        )
