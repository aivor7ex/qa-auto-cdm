...
ENDPOINT = "/vlans/all/count"

VLANS_ALL_COUNT_SCHEMA = {
    "type": "object",
    "properties": {
        "count": {"type": "integer"},
    },
...
# Осмысленная параметризация для тестирования эндпоинта /vlans/all/count
PARAMS = [
    ...
    pytest.param({"filter": '{"name": "native"}'}, 200, id="P04: filter_by_name_native"),
    pytest.param({"filter": '{"interface": "eth-0-1"}'}, 200, id="P05: filter_by_interface"),
    pytest.param({"search": "native"}, 200, id="P06: search_native"),
    pytest.param({"search": "eth"}, 200, id="P07: search_eth_interfaces"),
    pytest.param({"q": "vlan"}, 200, id="P08: query_text"),
    pytest.param({"name": "native"}, 200, id="P09: filter_by_name_param"),
    pytest.param({"vlanId": "0"}, 200, id="P10: filter_by_vlanId_param"),
    pytest.param({"interface": "eth-0-1"}, 200, id="P11: filter_by_interface_param"),
    pytest.param({"filter": '{"vlanId": 0, "name": "native"}'}, 200, id="P12: multiple_filters"),
    pytest.param({"vlanId": "0", "interface": "eth-0-1"}, 200, id="P13: param_filters"),
    pytest.param({"filter": '{"l2mode": 0}'}, 200, id="P14: filter_by_l2mode"),
    pytest.param({"filter": '{"stp": false}'}, 200, id="P15: filter_by_stp_false"),
    pytest.param({"filter": '{"mtu": 1500}'}, 200, id="P16: filter_by_mtu"),
    pytest.param({"mtu": "1500"}, 200, id="P17: filter_by_mtu_param"),
    pytest.param({"MAC": "54:5a:00:a1:52:05"}, 200, id="P18: filter_by_mac_param"),
    pytest.param({"l2mode": "0"}, 200, id="P19: filter_by_l2mode_param"),
    pytest.param({"stp": "false"}, 200, id="P20: filter_by_stp_param"),
    pytest.param({"sort": "vlanId"}, 200, id="P21: sort_by_vlanId"),
    pytest.param({"sort": "name"}, 200, id="P22: sort_by_name"),
    pytest.param({"limit": "10"}, 200, id="P23: limit_10"),
    pytest.param({"skip": "5"}, 200, id="P24: skip_5"),
    pytest.param({"filter": "invalid_json"}, 400, id="P25: invalid_json_error"),
    ...
    pytest.param({"unsupported_param": "value"}, 200, id="P29: unsupported_param_ignored"),
]
...
def test_vlans_all_count_parametrized(api_client, params, expected_status):
    """
    Основной параметризованный тест для эндпоинта /vlans/all/count.
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            _check_types_recursive(data, VLANS_ALL_COUNT_SCHEMA)
