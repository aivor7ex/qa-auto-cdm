"""
Тесты для эндпоинта /compose-files/{id} сервиса csi-server.
...
# Схема ответа для compose-files/{id} на основе реального ответа API
COMPOSE_FILE_SCHEMA = {
    "type": "object",
    "properties": {
        "id": {"type": "string"},
...
TEST_PARAMS = [
    ...
    {"cache": "true"},
    {"cache": "false"},
    {"fields": "id,name", "pretty": "true"},
    {"include": "services", "exclude": "volumes"},
...
def _format_curl_command(api_client, endpoint, id_param, params, auth_token):
    ...
    full_url = f"{base_url.rstrip('/')}/{endpoint.lstrip('/')}/{id_param}"
...
def test_compose_files_id_parametrized(api_client, auth_token, param_index):
    """
    Параметризованный тест для эндпоинта /compose-files/{id}.
    ...
    # 2. Тестируем эндпоинт с полученным ID
    params = TEST_PARAMS[param_index]
    
    try:
        response = api_client.get(f"{ENDPOINT}/{valid_id}", params=params, headers=headers)
        ...
        _check_types_recursive(data, COMPOSE_FILE_SCHEMA)
...
def test_compose_files_id_negative(api_client, auth_token):
    """
    Негативные тесты для эндпоинта /compose-files/{id}.
    ...
    for invalid_id in invalid_ids:
        try:
            response = api_client.get(f"{ENDPOINT}/{invalid_id}", headers=headers)
