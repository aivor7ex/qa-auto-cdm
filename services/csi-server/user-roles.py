...
POSITIVE_CASES = [
    ...
    pytest.param({"use_auth_token": True}, {"page": "1"}, "", 200, id="valid-token-pagination"),
    ...
    pytest.param({"use_auth_token": True}, {"fields": "id,name"}, "", 200, id="valid-token-fields"),
    pytest.param({"use_auth_token": True, "X-Custom-Header": "test-value"}, None, "", 200, id="valid-token-custom-header"),
]
...
NEGATIVE_CASES = [
    ...
    pytest.param({"use_auth_token": True}, None, "/non-existent-endpoint", 500, id="non-existent-endpoint"),
...
def test_get_user_roles_positive(api_client, auth_token, attach_curl_on_fail, headers, params, endpoint_suffix, expected_status):
...
def test_get_user_roles_negative(api_client, auth_token, attach_curl_on_fail, headers, params, endpoint_suffix, expected_status):
    if "use_auth_token" in headers:
        headers["x-access-token"] = auth_token
        del headers["use_auth_token"]
    full_endpoint = ENDPOINT + endpoint_suffix
