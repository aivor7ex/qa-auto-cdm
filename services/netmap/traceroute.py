        ...
        "properties": {
            ...
            "hosts": {
                ...
                "items": {
                    ...
                    "properties": {
                        "address": {"type": "string"},
                        "hostnames": {"anyOf": [{"type": "array", "items": {"type": "string"}}, {"type": "null"}]},
                        "up": {"type": "boolean"},
                        "hop": {
                            "type": "array",
                            "items": {
                                ...
                                "properties": {
                                    "level": {"type": "integer"},
                                    "address": {"type": "string"},
                                    "hostname": {"anyOf": [{"type": "string"}, {"type": "null"}]},
                                },
                                "required": ["level", "address"],
                            }
                        },
                    },
                    "required": ["address", "up", "hop"],
                }
            },
        },
        "required": ["id", "network", "gw", "command", "hosts"],
    }
}
...
test_cases = [
    ...
    ({"limit": 10, "offset": 1, "filter": 'network=="10.0.0.0/8"'}, 422),
    ({"sort": "-gw", "filter": 'up==true'}, 422),
    ({"offset": 20, "sort": "id", "limit": 20}, 422),
    ({"filter": 'hostname.contains("server")'}, 422),
    ({"limit": 999, "offset": 1}, 422),
    ({"limit": -1, "offset": 1}, 422),
    ({"limit": "abc", "offset": 1}, 422),
    ({"limit": 1001, "offset": 1}, 422),
    ({"offset": -1}, 422),
    ({"offset": "abc"}, 422),
...
def test_traceroute_api(api_client, params, expected_status, attach_curl_on_fail):
    with attach_curl_on_fail(ENDPOINT, payload=params, method="GET"):
        response = api_client.get(ENDPOINT, params=params)
        assert response.status_code == expected_status
        if response.status_code == 200:
            response_data = response.json()
            _check_types_recursive(response_data, TRACEROUTE_SCHEMA)
