...
IGNORED_QUERY_PARAMS = [
    ...
    pytest.param({"active": "true"}, 400, id="active-true"),
    pytest.param({"active": "false"}, 400, id="active-false"),
...
def test_object_count_valid(api_client, params, attach_curl_on_fail):
    with attach_curl_on_fail(ENDPOINT, params, method="GET"):
        ...
        _check_types_recursive(value, RESPONSE_SCHEMA)

@pytest.mark.parametrize("params, expected_status", IGNORED_QUERY_PARAMS)
def test_object_count_invalid(api_client, params, expected_status, attach_curl_on_fail):
    with attach_curl_on_fail(ENDPOINT, params, method="GET"):
        try:
            response = api_client.get(ENDPOINT, params=params)
